...
Instructions to install [`ROpenCVLite`](https://github.com/swarm-lab/ROpenCVLite)
can be found [here](/ROpenCVLite//articles/install.html). Instructions on how to use 
[`ROpenCVLite`](https://github.com/swarm-lab/ROpenCVLite)
when developing packages can be found [here](/ROpenCVLite/articles/usage.html).
...
algorithms, you pretty much have to use [`OpenCV`](http://opencv.org/) nowadays. 
...
[imager](http://dahtah.github.io/imager/) for instance) but none of them can 
...
based on [`OpenCV`](http://opencv.org/). [`ROpenCVLite`](https://github.com/swarm-lab/ROpenCVLite)
facilitates the installation of [`OpenCV`](http://opencv.org/) in a convenient 
location for other packages to find it and compile against it. 

### Couldn't you just ship [`OpenCV`](http://opencv.org/)'s binaries with your package?

Yes, but... it is fairly difficult to ship binary files compiled against 
[`OpenCV`](http://opencv.org/) that will work for sure on someone else's computer. 
It is not [`OpenCV`](http://opencv.org/)'s fault; it is because there are so many 
...
is therefore to make sure that there is a copy of [`OpenCV`](http://opencv.org/) 
on each user's computer that has been compiled taking into account the 
specificities of said computer.

HOWEVER...

[`OpenCV`](http://opencv.org/) is notoriously difficult to compile and install 
...
play. This package tries as much as possible to cleanly compile [`OpenCV`](http://opencv.org/)
...
the core modules of the [`OpenCV`](http://opencv.org/) library but does not 
...
[`OpenCV`](http://opencv.org/), and (2) they do not always compile nicely.
